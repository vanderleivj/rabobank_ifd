...
@Component({
  selector: 'app-comercialization-current-harvest-list',
  templateUrl: './list.component.html',
  styleUrls: []
})
export class ListCommercializationCurrentHarvestComponent implements OnInit {
  ...
  public currentHarvests: Harvest[] = [];
  ...
  loadData() {
    this.spinnerService.requestStarted();
    this.harvestService
      .getAllCurrent(this.bidId, this.pagination, this.search.value)
      .subscribe((response) => {
        this.currentHarvests = response.result;
        console.log(this.currentHarvests);
  ...
  loadEdit(id: string) {
    this.currentHarvests.forEach(item => {
      if (item.id === id) {
        item.isEditing = true;
